...
export const DataBrowser = ({ resourceId, fields, apiUrl }: DataBrowserProps) => {
        ...
        <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
                        ...
                        <div className="space-y-4">
                            {Object.keys(fields).map((field) => (
                                <div key={field}>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{field} ({fields[field]})</label>
                                    {field === 'id' && !isNew ? (
                                        <input
                                            disabled
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={() => {
                            const schemaTemplate = Object.keys(fields).reduce((acc, key) => ({ ...acc, [key]: '' }), {});
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-gray-100">
                                ...
                                <tr key={item.id} className="hover:bg-gray-50/50 group">
                                    {Object.keys(fields).map((f) => (
                                        <td key={f} className="px-4 py-3 max-w-[200px] truncate">
                                            {typeof item[f] === 'object' ? JSON.stringify(item[f]) : String(item[f] || "")}
                                        </td>
                                    ))}
                            ...
                            {data.length === 0 && (
                                <tr>
                                    <td colSpan={Object.keys(fields).length + 1} className="px-4 py-8 text-center text-gray-400 italic">
